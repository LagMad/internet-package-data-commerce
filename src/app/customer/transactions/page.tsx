...
import { HistoryOutlined, SearchOutlined, FileTextOutlined, ReloadOutlined } from '@ant-design/icons';
import { useTransactions } from '@/hooks/useTransactions';
import { useAuth } from '@/contexts/AuthContext';
import { formatCurrency, formatDateShort, statusLabel } from '@/utils/formatters';
...
export default function CustomerTransactionsPage() {
  ...
  const filtered = transactions.filter((t) => {
    const matchesSearch = t.packageName.toLowerCase().includes(search.toLowerCase()) || 
    ...
    <div className="">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-white mt-1">Daftar semua transaksi paket data Anda</p>
        </div>
      ...
      <Card className="bg-cust-black overflow-hidden">
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-muted-foreground">Tidak ada riwayat transaksi yang ditemukan.</p>
              {(search || statusFilter !== 'all') && (
                <Button variant="link" onClick={() => {setSearch(''); setStatusFilter('all');}} className="text-cust-red mt-2">
                  Reset Filter
                </Button>
              )}
            </div>
                ...
                <TableBody>
                  {filtered.map((trx) => (
                    <TableRow key={trx.id} className="border-white/5 hover:bg-white/5 transition-colors">
                      ...
                      <TableCell>
                        <div className="font-medium text-white">{trx.packageName}</div>
                        <div className="text-xs text-muted-foreground">{trx.packageQuota}</div>
                      </TableCell>
                      <TableCell className="/80 text-white">{trx.phoneNumber}</TableCell>
                      ...
                      <TableCell className="text-center">
                        <Badge className='min-w-32' variant={statusVariant[trx.status] ?? 'outline'}>
